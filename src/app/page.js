    ...
    <main className={styles.main}>
      <div class={styles.container}>
        {loading ? (
          ...
          <ResultsContent results={results} setResults={setResults} />
        ) : (
          <InputContent
            setInput={setInput}
...
const InputContent = ({ setInput, fetchResults, emptySubmit }) => {
  return (
    <div class={styles.inputContent}>
      <div class={styles.inputWrapper}>
        <div>Explain&nbsp;</div>
        <input
          className={emptySubmit && styles.error}
          autoFocus
          type="text"
          onInput={(e) => setInput(event.target.value)}
        ></input>
        <div>, but use bananas...</div>
      </div>
      <button onClick={fetchResults}>Go bananas!</button>
    </div>
  );
};

const ResultsContent = ({ results, setResults }) => {
  return (
    <div class={styles.resultsContent}>
      <div class={styles.resultsTop}>
        <button className={styles.restartButton} onClick={() => setResults("")}>
          <svg
            width="32"
            height="32"
            viewBox="0 0 43 44"
      ...
      <div class={styles.resultsText}>{results}</div>
    </div>
  );
};
